...
interface PageData {
  verification: PoliticianVerification | null;
  error?: string;
}
...
export const handler: Handlers<PageData, AuthState> = {
  async GET(_req, ctx) {
    ...
    try {
      const res = await fetch(`${apiBase}/api/admin/politician-verifications/${id}`, {
    ...
    try {
      if (action === "approve") {
        const res = await fetch(
          `${apiBase}/api/admin/politician-verifications/${id}/approve`,
          {
            method: "PUT",
            headers: {
        ...
        if (!res.ok) {
          ...
          const reqRes = await fetch(
            `${apiBase}/api/admin/politician-verifications/${id}`,
            { headers: { Authorization: `Bearer ${ctx.state.accessToken}` } }
          );
      ...
      if (action === "reject") {
        ...
        const res = await fetch(
          `${apiBase}/api/admin/politician-verifications/${id}/reject`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
...
export default function PoliticianVerificationDetail({ data }: PageProps<PageData>) {
  ...
  if (!verification) {
    return (
      <Layout active="/verifications">
        ...
        <a href="/verifications?tab=politician" class="btn btn-ghost mt-4">
          ← 戻る
        </a>
      </Layout>
    );
    ...
    <Layout active="/verifications">
      <div class="space-y-6">
        <div class="flex items-center gap-4">
          <a href="/verifications?tab=politician" class="btn btn-ghost btn-sm">
            ← 戻る
          </a>
          <h1 class="text-3xl font-bold">政治家認証申請詳細</h1>
        </div>
        ...
        <div class="card bg-base-100 shadow">
          <div class="card-body">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              ...
              {verification.party && (
                <div>
                  <p class="text-sm opacity-70">所属政党</p>
                  <p class="font-medium">{verification.party}</p>
                </div>
              )}
              <div>
