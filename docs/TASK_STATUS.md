...
| 市区町村データ投入 (1,794件) | ✅ 完了 | seed スクリプトで実行済 |
| ダミーデータ投入 | ✅ 完了 | 政治家5名、団体6件、選挙3件等 |

### Ledger DB (Supabase)

| タスク | 状態 | SQL |
|--------|------|-----|
| `ledger_year_closures` テーブル作成 | 🔲 未実行 | 下記参照 |
...
| 7 | 選挙区マスタ初期データ | 🔲 未着手 | 衆院289区 + 参院45区 |
...
| # | タスク | 状態 | 詳細 |
|---|--------|------|------|
| 1 | `ledger_year_closures` テーブル作成 | 🔲 SQL実行待ち | 年度締め機能用 |
