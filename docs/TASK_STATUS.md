...
| 5   | Polimoney JSON 形式 API       | ✅ 完了   | `/api/v1/polimoney/*` 実装済み    |
...
| 7   | 選挙区マスタ初期データ       | ✅ 完了   | 全4,016区（HR/HC/PG/PA/GM/CM） |
| 8   | Admin UI: リクエスト承認機能 | ✅ 完了   | 選挙・団体・ロック解除   |
| 9   | Admin UI: ダッシュボード統計 | ✅ 完了   | 基本的な統計表示         |
...
| 2   | Hub 同期機能（送信側）              | 🔲 未着手 | 承認時の自動同期 + 手動再同期 |
...
1. **Ledger**: Hub 同期機能（送信側）実装
2. **Hub**: Polimoney JSON 形式 API
3. **Hub**: 選挙区マスタ初期データ投入
...
| GET    | `/api/v1/polimoney/ledgers/:id/journals`           | 台帳ID指定でデータ取得 |
...
- 2024-12-14: Polimoney JSON 形式 API 実装、選挙区マスタ4,016区登録
