...
│  ┌─────────────── 共通識別子 API ───────────────┐  ┌─── 公開データ API ───┐
│  │ ┌────────────┐ ┌──────────────┐ ┌──────────┐ │  │ ┌────────────────┐  │
│  │ │Politicians │ │Organizations │ │Elections │ │  │ │ Public Ledgers │  │
│  │ │ API        │ │ API          │ │ API      │ │  │ │ API            │  │
│  │ └────────────┘ └──────────────┘ └──────────┘ │  │ ├────────────────┤  │
│  └──────────────────────┬───────────────────────┘  │ │ Public Journals│  │
│                         │                          │ │ API            │  │
...
└───────────────────────────────────────┬─────────────────────────────────┘
                   ...
                   │                                         │
                   │ [公開データ]                            │
                   │ - public_ledgers      (公開用台帳)      │
                   │ - public_journals     (公開用仕訳)      │
                   │ - ledger_change_logs  (変更履歴)        │
                   └─────────────────────────────────────────┘
```
...
│   ├── index.ts              # エントリポイント
│   ├── routes/
│   │   ├── politicians.ts    # 政治家 API         ✅ 実装済
│   │   ├── organizations.ts  # 政治団体 API       ✅ 実装済
│   │   ├── elections.ts      # 選挙 API           ✅ 実装済
│   │   ├── ledgers.ts        # 公開台帳 API       ⏳ 予定
│   │   ├── journals.ts       # 公開仕訳 API       ⏳ 予定
│   │   ├── changes.ts        # 変更履歴 API       ⏳ 予定
...
│ 📋 変更履歴                                        ✕ 閉じる│
├─────────────────────────────────────────────────────────────┤
│  📅 2024/03/01 10:30  仕訳1件を修正           ▶ 詳細       │
│  📅 2024/02/25 14:15  仕訳2件を追加           ▶ 詳細       │
│  📅 2024/01/20 09:00  初回同期（15件）                      │
└─────────────────────────────────────────────────────────────┘
```

**Level 2: 変更詳細（日付クリック後）**

```
┌─────────────────────────────────────────────────────────────┐
│ 📋 変更履歴 > 2024/03/01 10:30                   ◀ 戻る    │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  ✚ 追加された仕訳                                           │
│  ─────────────────────────────────────────────────────────  │
│  2024/02/20  事務所賃料      ¥80,000                        │
│  2024/02/15  交通費          ¥5,000                         │
│                                                              │
│  ✎ 修正された仕訳                                           │
│  ─────────────────────────────────────────────────────────  │
│  2024/01/15  ポスター印刷                                   │
...
透明性のため、重要フィールドの差分（before/after）を保存：

| フィールド     | 保存 | 理由             |
| -------------- | ---- | ---------------- |
| `amount`       | ✅   | 金額変更は重要   |
| `description`  | ✅   | 摘要変更は重要   |
| `contact_name` | ✅   | 支出先変更は重要 |
| `date`         | ✅   | 日付変更は怪しい |
| `account_code` | ❌   | 科目変更は稀     |
